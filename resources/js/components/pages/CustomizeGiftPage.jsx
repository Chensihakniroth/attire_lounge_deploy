                    ...
                    <div key={i} className="flex flex-col items-center z-10">
                        <motion.div
                            className={`w-2 h-2 rounded-full mb-3 transition-all duration-700 ${
                                currentStep >= i + 1
                                    ? 'bg-attire-accent scale-150 shadow-[0_0_10px_rgba(212,168,76,0.5)]'
                                    : 'bg-white/30'
                            }`}
                        />
                        <span
                            className={`text-[9px] uppercase tracking-[0.3em] font-black transition-colors duration-500 ${
                                currentStep === i + 1
                                    ? 'text-white'
                                    : 'text-white/40'
                            }`}
...
const SelectionCard = ({ item, isSelected, onSelect, isOutOfStock }) => {
    return (
        ...
        >
            <div
                className={`aspect-[3/4] rounded-2xl overflow-hidden transition-all duration-700 border-2 ${
            ...
            >
                ...
                <div className="absolute bottom-4 left-4 right-4">
                    <p className="text-[10px] font-bold text-attire-accent uppercase tracking-widest mb-1">
...
const CustomInput = ({ label, icon: Icon, error, ...props }) => (
    <div className="space-y-2">
        <label className="text-[10px] font-black text-white/60 uppercase tracking-[0.3em] ml-1">
            {label}
        </label>
...
const CustomizeGiftPage = () => {
    ...
    useEffect(() => {
        if (contentRef.current && step > 1) {
            // The function to safely scroll to the main content area
            const scrollToContent = () => {
                const element =
                    document.getElementById('step-indicator-area') ||
                    contentRef.current;
                const yOffset = -80; // Breathing room
                const elementTop =
            ...
            // Start scrolling immediately so it feels responsive
            scrollToContent();

            // Prevent the "snap to footer" browser bug:
            // Browsers try to anchor to the bottom when large elements (like Step 1) unmount.
            // We run it again precisely after the framer-motion exit transition ends (600ms)
            const timer = setTimeout(scrollToContent, 650);
    ...
    const validateStep1 = () => {
        ...
        if (!formData.recipient_name.trim())
            errors.recipient_name = 'Who is receiving this?';
        ...
        <div className="min-h-screen bg-attire-navy text-white selection:bg-attire-accent selection:text-black">
            ...
            <div className="relative z-10 max-w-[1400px] mx-auto px-6 py-28 sm:py-40">
                ...
                <header className="max-w-3xl mx-auto text-center mb-24">
                    ...
                    >
                        Tailor a masterpiece for those who appreciate the finer
                        things.
                    </motion.p>
                </header>
                ...
                >
                                ...
                                {step === 1 && (
                                    <div className="space-y-24 pb-20">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                                {giftOptions.ties.map((t) => (
                                                    <SelectionCard
                                                        ...
                                                        isOutOfStock={outOfStockItems.includes(
                                                            t.id
                                                        )}
                                                    />
                                                ))}
                                        ...
                                        <div className="space-y-12">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                                {giftOptions.pocketSquares.map(
                                                    (p) => (
                                                        <SelectionCard
                                                            key={p.id}
                                                            item={p}
                                                            isSelected={
                                                                selectedPocketSquare?.id ===
                                                                p.id
                                                            }
                                                            onSelect={() =>
                                                                setSelectedPocketSquare(
                                                                    p
                                                                )
                                                            }
                                                            isOutOfStock={outOfStockItems.includes(
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                                            <section className="space-y-8">
                                                <div className="flex items-center gap-4">
                                                    <span className="text-3xl font-serif text-white/30">
                                                        01
                                                    </span>
                                                    <h3 className="text-sm font-black uppercase tracking-[0.3em] text-attire-accent">
                                                        The Sender
                                                    </h3>
                                                </div>
                                            ...
                                            <section className="space-y-8">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <h3 className="text-sm font-black uppercase tracking-[0.3em] text-white/60">
                                                        The Recipient
                                                    </h3>
                                                </div>
                                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-12 bg-white/[0.02] border border-white/10 p-12 rounded-[2.5rem] backdrop-blur-3xl relative overflow-hidden">
                                        ...
                                        <div className="flex items-center gap-6 mb-12">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-white/60 mt-1">
                                                    Review your private request
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 border-y border-white/10 py-12">
                                            <div className="space-y-6">
                                                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-attire-accent">
                                                    Logistics
                                                </p>
                                    ...
                                    <div className="text-center space-y-12 py-20">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <p className="text-white/80 font-light max-w-md mx-auto">
                                                Thank you, {formData.name}. Our
                                                concierge will review your
                                                curation and contact you
                                                shortly.
                                            </p>
                                        </div>
                                        <button
                    ...
                    <div className="lg:col-span-5 xl:col-span-4 sticky top-40 space-y-8 hidden lg:block">
                        <div className="bg-white/[0.02] border border-white/10 p-10 rounded-[2.5rem] backdrop-blur-3xl space-y-10">
                            ...
                            <div className="pt-10 border-t border-white/10 flex justify-between items-end">
                                ...
                                <div className="text-right">
                                    ...
                                    <span className="text-[10px] text-white/60 font-bold uppercase tracking-widest italic">
                                        Complimentary
                                    </span>
                                </div>
                            </div>
                        ...
                        <div className="flex items-center gap-4 p-6 bg-white/[0.01] rounded-3xl border border-white/10 border-dashed opacity-60">
                            <Info
                                size={16}
                                className="text-attire-accent shrink-0"
                            />
                            <p className="text-[9px] font-medium uppercase tracking-widest leading-loose text-white/80">
                                Items are curated specifically for your request.
                                Final availability confirmed upon contact.
                            </p>
                        </div>
                    </div>
                </div>
...
const PreviewItem = ({ label, item }) => (
    <div className="flex items-center gap-6 group">
        <div
            className={`w-16 h-20 rounded-xl overflow-hidden bg-white/5 border border-white/10 transition-all duration-700 ${item ? 'scale-100' : 'scale-90 opacity-20'}`}
        >
            {item && (
        ...
        <div className="space-y-1">
            ...
            <p
                className={`text-sm font-serif transition-colors duration-500 ${item ? 'text-white' : 'text-white/20 italic'}`}
            >
                {item ? item.name : 'Awaiting Selection...'}
            </p>
            {item && (
                <p className="text-[10px] font-mono text-attire-accent font-bold">
                    ${item.price}
                </p>
            )}
        </div>
