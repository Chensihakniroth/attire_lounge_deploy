...
import {
    X, Plus, Edit, Trash2, 
    Hash, DollarSign, Layers, Check, CheckCircle,
            ...
            >
                ...
                <td className="px-6 py-3 border-l-2 border-black/15 dark:border-[#30363d] overflow-hidden">
                    <div className="flex items-center gap-3 leading-tight truncate">
                        ...
                        <span className={`font-black uppercase tracking-wider ${performanceMode ? '' : 'group-hover:text-[#0d3542] dark:group-hover:text-[#58a6ff] transition-colors'} text-[14px] truncate flex items-center gap-2 ${!d.is_active ? 'text-gray-400 opacity-50 line-through' : 'text-gray-900 dark:text-[#c9d1d9]'}`}>
                            {d.name}
        ...
        <div className="p-8 space-y-8 font-sans bg-[#fdfdfc] dark:bg-[#010409] min-h-screen text-gray-900 dark:text-[#c9d1d9]">
            {toast && (
                <div className="fixed top-6 right-6 z-50 animate-in slide-in-from-top-4 fade-in">
                    <div className={`px-6 py-4 rounded-xl border flex items-center gap-3 font-bold text-[11px] uppercase tracking-widest shadow-2xl ${toast.type === 'error' ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400' : 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-900/50 text-emerald-600 dark:text-emerald-400'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    { label: 'Total Drinks', value: stats.total, icon: <Coffee size={20} />, color: 'text-[#0d3542] dark:text-[#58a6ff]' },
                    { label: 'Active Items', value: stats.total - stats.outOfStock, icon: <CheckCircle size={20} />, color: 'text-green-500' },
                    { label: 'Low Stock', value: stats.lowStock, icon: <AlertCircle size={20} />, color: 'text-amber-500' },
                    { label: 'Out of Stock', value: stats.outOfStock, icon: <AlertCircle size={20} />, color: 'text-red-500' }
                ].map((stat, idx) => (
                    ...
                    <thead>
                        <tr className="bg-black/[0.02] dark:bg-[#0d1117] border-b border-black/5 dark:border-[#30363d]">
                            <th className="w-16 px-6 py-5 text-center">
                                <button onClick={handleSelectAll} className={`w-5 h-5 rounded-md border flex items-center justify-center transition-all mx-auto ${selectedIds.size === filteredDrinks.length && filteredDrinks.length > 0 ? 'bg-[#0d3542] dark:bg-[#58a6ff] border-[#0d3542] dark:border-[#58a6ff]' : 'bg-black/5 dark:bg-[#0d1117] border-black/10 dark:border-[#30363d] hover:border-[#0d3542]/40'}`}>
                                    {selectedIds.size === filteredDrinks.length && filteredDrinks.length > 0 && <Check size={12} className="text-white dark:text-black" />}
                                </button>
                            </th>
                            <th className="w-16 px-6 py-5 text-center text-xs font-bold uppercase tracking-[0.2em] text-gray-500 dark:text-[#8b949e]/40 whitespace-nowrap">STS</th>
                            <th className="w-32 px-6 py-5 text-center text-xs font-bold uppercase tracking-[0.2em] text-gray-500 dark:text-[#8b949e]/40 whitespace-nowrap">SKU</th>
                            <th className="w-auto px-6 py-5 text-left text-xs font-bold uppercase tracking-[0.2em] text-gray-500 dark:text-[#8b949e]/40 whitespace-nowrap">Beverage</th>
                            <th className="w-40 px-6 py-5 text-center text-xs font-bold uppercase tracking-[0.2em] text-gray-500 dark:text-[#8b949e]/40 whitespace-nowrap">Category</th>
                            <th className="w-32 px-6 py-5 text-right text-xs font-bold uppercase tracking-[0.2em] text-gray-500 dark:text-[#8b949e]/40 whitespace-nowrap">Stock</th>
                            <th className="w-32 px-6 py-5 text-center text-xs font-bold uppercase tracking-[0.2em] text-gray-500 dark:text-[#8b949e]/40 whitespace-nowrap">Price</th>
                            <th className="w-32 px-6 py-5 text-right text-xs font-bold uppercase tracking-[0.2em] text-gray-500 dark:text-[#8b949e]/40 whitespace-nowrap">Actions</th>
                        </tr>
                    </thead>
                        ...
                        {(isLoading && drinks.length === 0) ? (
                            Array.from({ length: 8 }).map((_, i) => (
                                <tr key={i} className="animate-pulse">
                                    <td className="px-6 py-4 text-center"><div className="w-5 h-5 rounded bg-gray-200 dark:bg-white/5 mx-auto" /></td>
                                    <td className="px-6 py-4 text-center"><div className="w-2 h-2 rounded-full bg-gray-200 dark:bg-white/5 mx-auto" /></td>
                                    <td className="px-6 py-4 text-center"><div className="h-4 w-16 mx-auto rounded bg-gray-200 dark:bg-white/5" style={{ animationDelay: `${i * 80}ms` }} /></td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-lg bg-gray-200 dark:bg-white/5" />
                                            <div className={`h-4 rounded bg-gray-200 dark:bg-white/5`} style={{ width: `${100 + Math.random() * 120}px`, animationDelay: `${i * 80 + 20}ms` }} />
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-center"><div className="h-5 w-20 mx-auto rounded-lg bg-gray-200 dark:bg-white/5" style={{ animationDelay: `${i * 80 + 40}ms` }} /></td>
                                    <td className="px-6 py-4 text-right"><div className="h-4 w-10 ml-auto rounded bg-gray-200 dark:bg-white/5" style={{ animationDelay: `${i * 80 + 60}ms` }} /></td>
                                    <td className="px-6 py-4 text-center"><div className="h-4 w-14 mx-auto rounded bg-gray-200 dark:bg-white/5" style={{ animationDelay: `${i * 80 + 80}ms` }} /></td>
                                    <td className="px-6 py-4 text-right"><div className="h-8 w-8 ml-auto rounded-lg bg-gray-200 dark:bg-white/5" style={{ animationDelay: `${i * 80 + 100}ms` }} /></td>
                                </tr>
                            ))
                            ...
                            <tr>
                                <td colSpan="8" className="px-6 py-20 text-center opacity-40">
                                    ...
                                    <h3 className="text-sm font-bold uppercase tracking-widest text-gray-900 dark:text-white mb-1">No Drinks Found</h3>
                                    <p className="text-xs text-gray-500 uppercase tracking-widest">Try adjusting your search or filters.</p>
                                </td>
                            </tr>
                        ) : (
            ...
            <div className="flex items-center justify-between pb-10">
                <p className="text-xs font-bold text-gray-500 dark:text-[#8b949e]/40 uppercase tracking-widest">
                    Showing {filteredDrinks.length} of {meta.total || drinks.length} drinks
                </p>
            ...
            {/* Editing Form Modal */}
            <ModernModal isOpen={isModalOpen} onClose={closeModal} title={editingDrink ? 'Edit Drink' : 'New Drink'} icon={Coffee} overflowVisible={true}>
                    ...
                    <form onSubmit={(e) => {
                        ...
                        <div className="flex items-center gap-4 p-4 bg-white dark:bg-[#161b22] border border-black/5 dark:border-[#30363d] rounded-xl shadow-sm">
                            ...
                            <div className="flex-1">
                                <h4 className="text-[12px] font-bold uppercase tracking-widest text-gray-900 dark:text-white mb-1">Drink Image</h4>
                                <p className="text-[11px] font-medium text-gray-500 dark:text-[#8b949e]">Upload a high-quality picture for the POS menu (Optional).</p>
                                {formData.image_path && (
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5 col-span-2">
                                <label className="block text-[10px] font-bold text-gray-500 dark:text-[#8b949e]/80 uppercase tracking-[0.15em] ml-0.5">Name</label>
                                <input required value={formData.name} onChange={e => setFormData(f => ({ ...f, name: e.target.value }))} className="w-full bg-white dark:bg-[#0d1117] text-gray-900 dark:text-white px-4 py-3 rounded-xl border border-black/5 dark:border-[#30363d] focus:border-[#0d3542] dark:focus:border-[#58a6ff] outline-none text-sm font-bold tracking-wide transition-colors" placeholder="Latte, Mocha..." />
                            </div>
                            <div className="space-y-1.5">
                                <label className="block text-[10px] font-bold text-gray-500 dark:text-[#8b949e]/80 uppercase tracking-[0.15em] ml-0.5">Category</label>
                                {isCreatingCategory ? (
                                    <div className="flex gap-2">
                                        ...
                                        />
                                        <button type="button" onClick={() => { if (newCategoryName.trim()) { setFormData({...formData, category: newCategoryName.trim()}); setIsCreatingCategory(false); setNewCategoryName(''); } }} className="px-3 py-3 bg-[#0d3542] dark:bg-[#58a6ff] text-white dark:text-black rounded-xl hover:opacity-90 transition-all"><Check size={16} /></button>
                                        <button type="button" onClick={() => { setIsCreatingCategory(false); setNewCategoryName(''); }} className="px-3 py-3 bg-black/5 dark:bg-white/5 text-gray-500 hover:bg-red-500/10 hover:text-red-500 rounded-xl transition-all"><X size={16} /></button>
                                    </div>
                                ) : (
                                    <BespokeSelect 
                                        ...
                                        options={[
                                            ...categories,
                                            { label: '+ Create New Category', value: 'NEW_CATEGORY', isAction: true }
                                        ]}
                            ...
                            <div className="space-y-1.5">
                                <label className="block text-[10px] font-bold text-gray-500 dark:text-[#8b949e]/80 uppercase tracking-[0.15em] ml-0.5">Price</label>
                                <input type="number" step="0.01" required value={formData.price} onChange={e => setFormData(f => ({ ...f, price: e.target.value }))} className="w-full bg-white dark:bg-[#0d1117] text-gray-900 dark:text-white px-4 py-3 rounded-xl border border-black/5 dark:border-[#30363d] focus:border-[#0d3542] dark:focus:border-[#58a6ff] outline-none text-sm font-bold tracking-wide transition-colors" placeholder="0.00" />
                            </div>
                            <div className="space-y-1.5">
                                <label className="block text-[10px] font-bold text-gray-500 dark:text-[#8b949e]/80 uppercase tracking-[0.15em] ml-0.5">Stock</label>
                                <input type="number" disabled={formData.is_service} value={formData.stock_qty} onChange={e => setFormData(f => ({ ...f, stock_qty: e.target.value }))} className="w-full bg-white dark:bg-[#0d1117] text-gray-900 dark:text-white px-4 py-3 rounded-xl border border-black/5 dark:border-[#30363d] focus:border-[#0d3542] dark:focus:border-[#58a6ff] outline-none text-sm font-bold tracking-wide transition-colors disabled:opacity-50" placeholder={formData.is_service ? '∞' : '0'} />
                            </div>
                            <div className="space-y-1.5">
                                <label className="block text-[10px] font-bold text-gray-500 dark:text-[#8b949e]/80 uppercase tracking-[0.15em] ml-0.5">SKU (Optional)</label>
                                <input value={formData.sku} onChange={e => setFormData(f => ({ ...f, sku: e.target.value }))} className="w-full bg-white dark:bg-[#0d1117] text-gray-900 dark:text-white px-4 py-3 rounded-xl border border-black/5 dark:border-[#30363d] focus:border-[#0d3542] dark:focus:border-[#58a6ff] outline-none text-sm font-bold tracking-wide transition-colors" placeholder="Auto-gen if empty" />
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3 pt-4 border-t border-black/5 dark:border-[#30363d]">
                            <button type="button" onClick={closeModal} className="flex-1 py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest border border-black/10 dark:border-[#30363d] hover:bg-black/5 dark:hover:bg-white/5 transition-colors">Cancel</button>
                            <button type="submit" disabled={isSaving} className="flex-[2] py-3 bg-[#0d3542] dark:bg-[#58a6ff] text-white dark:text-black rounded-xl text-[11px] font-bold uppercase tracking-widest hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2 shadow-none">
